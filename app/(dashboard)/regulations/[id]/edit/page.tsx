...
export default function EditRegulationPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Erro ao atualizar regulacao')
        ...
        await Promise.all(
          documents.map(async (doc) => {
            try {
              ...
              const uploadResponse = await fetch(`/api/regulations/${regulationId}/attachments`, {
                method: 'POST',
                body: formData,
              })
      ...
      toast.success('Regulacao atualizada com sucesso!')
      router.push('/regulations')
    } catch (error) {
      console.error('Erro ao atualizar regulacao:', error)
      toast.error('Erro ao atualizar regulacao')
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <PageHeader
        title="Editar Regulacao"
        description={`Edite as informacoes em ${steps.length} etapas`}
        backHref="/regulations"
      />
